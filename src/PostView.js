...
const navigate=useNavigate();
    ...
    const dek =post?.body;
    const image =post?.imageLink;
  return (
    <div className="m-3" style={{ fontFamily: 'Roboto, sans-serif' }}>
      <Card style={{ width: '80vw', left: '10vw', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
        <Card.Header style={{ backgroundColor: 'rgba(0, 0, 0, 0.1)',textAlign:'center' }}>
          <b style={{ color: '#333' }}>{title}</b>
        </Card.Header>
        <Card.Body>
          <Row className="d-flex justify-content-between" style={{position:'relative'}}>
            <Col md={2}>
              <img
                src={image}
                alt={''}
            ...
            <Col md={7} className="mx-4 d-flex flex-column justify-content-between" style={{ color: 'rgba(0, 0, 0, 0.6)' }}>
              <div>
              <span>{dek}</span>
              </div>
              <div style={{textAlign:'end'}}>
              <Button style={{backgroundColor:'rgb(102, 153, 153)'}}
              onClick={()=>{
